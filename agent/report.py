...
def report(symbols: list[str]) -> None:
    ...
    for symbol in symbols:
        ...
        print(f"    Bull indicators : {signal.get('bull_signals', '?')}/8")
        print(f"    Bear indicators : {signal.get('bear_signals', '?')}/8")
        print(f"    Regime          : {signal.get('regime', '?')}")
        print(f"    RSI14           : {signal.get('rsi', '?')}")
        print(f"    MACD hist       : {signal.get('macd_hist', '?')}")
        print(f"    StochRSI K      : {signal.get('stoch_rsi_k', '?')}")
        print(f"    ADX             : {signal.get('adx', '?')}")
